    ...
    <section className="py-8 sm:py-12 space-y-8 md:space-y-16">
      ...
      <div className="container grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
        {ClientSList.map((client) => (
          <div
            key={client.id}
            className="flex flex-col space-y-3 sm:space-y-4 justify-center items-center text-center group bg-secondary p-8 text-white hover:bg-primary duration-300"
          >
            {/* <div className="w-full h-full group-hover:rotate-x-180">
              <img
                src={client.image}
                alt={client.name}
            ...
            <p className="text-lg md:text-xl">{client.designation}</p>
            <p className="text-lg md:text-xl">{client.review}</p>
            <div className="flex gap-1 text-xl">{renderStars(client.rate)}</div>
          </div>
        ))}
      </div>
    </section>
