...
const Serve = () => {
  return (
    <section className="py-10 sm:py-20 space-y-8 md:space-y-10">
      ...
      <div className="container px-0 flex flex-wrap justify-center items-center gap-4 md:gap-3">
        {data.map((item) => (
          <div
            key={item.id}
            className="w-36 h-48 flex flex-col justify-center items-center text-center  border border-transparent bg-white group hover:bg-primary hover:scale-105 transition duration-300"
          >
            <div className="h-16 w-16 bg-white rounded-full text-primary text-4xl flex justify-center items-center border border-gray-300 shadow-inner transition duration-300">
              {item.icon}
            </div>
